...
class _PDFViewerState extends State<PDFViewer> {
  ...
  void initState() {
    super.initState();
    final List<int> _count = <int> [widget.document.count];
    _pages = _count.cast();
  ...
  Future<void> _animateToPage({int page}) => _pageController.animateToPage(page ?? _pageNumber - 1,
        duration: animationDuration, curve: animationCurve);

  Future<void> _jumpToPage({int page}) async => _pageController.jumpToPage(page ?? _pageNumber - 1);
  ...
  Widget _drawIndicator() {
    final Widget child = GestureDetector(
        ...
        child: Container(
            padding:
                const EdgeInsets.only(top: 4.0, left: 16.0, bottom: 4.0, right: 16.0),
      ...
      body: Stack(
        children: <Widget>[
          PageView.builder(
            ...
            itemBuilder: (BuildContext context, int index) => _pages[index] ?? Center(
                    child:
                        widget.progressIndicator ?? const CircularProgressIndicator(),
                  ),
          ),
          if (widget.showIndicator && !_isLoading) _drawIndicator() else Container(),
        ],
      ),
